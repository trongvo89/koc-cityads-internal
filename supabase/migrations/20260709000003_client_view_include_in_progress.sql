-- Fix: clients saw no KOCs because the simplified board writes
-- operation_status = 'in_progress', but the client view whitelist still only
-- listed the retired granular statuses (sent_to_client, ...). Recreate the view
-- with 'in_progress' included so active KOCs on the board are visible to clients.
-- 'cancelled' / 'draft' / 'failed' remain hidden.
