-- Support multiple final video links per KOC (1 KOC may produce several videos)
-- and expose them to clients through client_campaign_kocs_view.

-- 1. Convert final_link TEXT -> TEXT[] preserving existing single values.
ALTER TABLE public.campaign_kocs
  ALTER COLUMN final_link TYPE TEXT[]
  USING (CASE
    WHEN final_link IS NULL OR trim(final_link) = '' THEN NULL
    ELSE ARRAY[final_link]
  END);

-- 2. Recreate client view to include final_link (array of video links).
